...
### En-têtes de sécurité (`_headers`) + CSP optionnelle

`_headers` à la racine envoie : `X-Frame-Options: DENY`,
`X-Content-Type-Options: nosniff`, `Referrer-Policy:
strict-origin-when-cross-origin`, `Permissions-Policy:
camera=(), microphone=(), geolocation=()`. Appliqué automatiquement par
Cloudflare Pages (pas d'effet en local).

**CSP** : un bloc `Content-Security-Policy` complet est fourni mais
**commenté** — à activer (retirer les `#`) seulement après test, puis
vérifier la console navigateur. Il a été validé automatiquement le
2026-09-03 (zéro violation) avec les domaines exacts du site :
`www.googletagmanager.com`, `www.clarity.ms` + `scripts.clarity.ms`,
`*.google-analytics.com`, `*.clarity.ms`, `docs.google.com` +
`*.googleusercontent.com` (⚠️ le Sheet CMS redirige ses CSV vers
`doc-…-sheets.googleusercontent.com` : sans ce domaine, tout le contenu
CMS resterait vide), `hook.eu1.make.com`, Google Fonts. Consultez les
commentaires du fichier pour les détails.
